...
trackData = {
    "y": {"description": "ground truth value", "type": bool},
    "chrom": {"description": "Chromosome", "type": str, "default": "ignore"},
    "pos": {"description": "Position (1-based)", "type": int, "default": "ignore"},
    "ref": {
    ...
    "type": {
        ...
        "hcdiff_transformation": lambda x: (
            "SNV" if x == "SNV" else {"INS": "DEL", "DEL": "INS"}[x]
        ),
    },
    "length": {
    ...
    "istv": {
        ...
        "derive": lambda x: (
            ...
            if x["type"] != "SNV"
                ...
                0
                if (x["ref"], x["alt"]) in TRANSITIONS
                else 1 if (x["ref"], x["alt"]) in TRANSVERSIONS else 0.5
            )
        ),
    },
    "annotype": {
    ...
    "consequence": {
        ...
        "hcdiff_transformation": lambda x: (
            x
            if x not in ["STOP_LOST", "STOP_GAINED"]
            else {"STOP_LOST": "STOP_GAINED", "STOP_GAINED": "STOP_LOST"}[x]
        ),
    },
    "consscore": {
    ...
    "geneid": {"description": "ENSEMBL GeneID", "type": str, "default": "ignore"},
    "featureid": {
    ...
    "polyphencat": {
        ...
        "derive": lambda x: (
            x["polyphencat"]
            if (x["type"] != "SNV")
            or (x["consequence"] != "NON_SYNONYMOUS")
            or (x["polyphencat"] != "NA")
            else "unknown"
        ),
    },
    ...
    "mw-relfreq1": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["freq100bp"]) / float(x["freq1000bp"]))
        ),
        "na_value": -0.5,
    },
    "mw-relfreq2": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["freq100bp"]) / float(x["freq10000bp"]))
        ),
        "na_value": -1.5,
    },
    "mw-relfreq3": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["freq1000bp"]) / float(x["freq10000bp"]))
        ),
        "na_value": -1,
    },
    "mw-relrare1": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["rare100bp"]) / float(x["rare1000bp"]))
        ),
        "na_value": -1,
    },
    "mw-relrare2": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["rare100bp"]) / float(x["rare10000bp"]))
        ),
        "na_value": -2,
    },
    "mw-relrare3": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["rare1000bp"]) / float(x["rare10000bp"]))
        ),
        "na_value": -1,
    },
    "mw-relsngl1": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["sngl100bp"]) / float(x["sngl1000bp"]))
        ),
        "na_value": -1,
    },
    "mw-relsngl2": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["sngl100bp"]) / float(x["sngl10000bp"]))
        ),
        "na_value": -2,
    },
    "mw-relsngl3": {
        ...
        "derive": lambda x: (
            0
            if x["type"] != "SNV"
            else math.log10(float(x["sngl1000bp"]) / float(x["sngl10000bp"]))
        ),
        "na_value": -1,
    },
    "dbscsnv-ada_score": {
    ...
    "regseq_positivemax": {
        ...
        "hcdiff_derive": lambda x: min(
            [float(i) if float(i) < 0 else 0 for i in x.values()]
        ),
        "transformation": abs,
        "type": float,
        "na_value": 0,
    ...
    "regseq_positivemean": {
        ...
        "copy": "regseq_positivemax",
        "derive": lambda x: statistics.mean(
            [float(i) if float(i) > 0 else 0 for i in x.values()]
        ),
        "hcdiff_derive": lambda x: statistics.mean(
            [float(i) if float(i) < 0 else 0 for i in x.values()]
        ),
        "hcdiff_na_value": 0,
    },
    ...
    "regseq_positivestd": {
        ...
        "copy": "regseq_positivemax",
        "derive": lambda x: statistics.stdev(
            [float(i) if float(i) > 0 else 0 for i in x.values()]
        ),
        "hcdiff_derive": lambda x: statistics.stdev(
            [float(i) if float(i) < 0 else 0 for i in x.values()]
        ),
        "hcdiff_na_value": 0,
    },
    ...
    "regseq_positive": {
        ...
        "hcdiff_derive": lambda x: (
            float(x["regseq7"]) if float(x["regseq7"]) < 0 else 0
        ),
        "transformation": abs,
        "type": float,
        "na_value": 0,
    ...
    "nuccomb": {
        ...
        "categories": [
            "%s-%s" % (a, b)
            for a in NUCLEOTIDES_N[:-1]
            for b in NUCLEOTIDES_N[:-1]
            if a != b
        ]
        + ["N-N"],
        "dependencies": ["ref", "alt", "type"],
        "derive": lambda x: (
            "N-N" if x["type"] != "SNV" else "%s-%s" % (x["ref"], x["alt"])
        ),
        "na_value": "N-N",
        "hcdiff_derive": lambda x: (
            "N-N" if x["type"] != "SNV" else "%s-%s" % (x["alt"], x["ref"])
        ),
    },
    "aacomb": {
        ...
        "derive": lambda x: (
            "NA" if x["type"] != "SNV" else "%s-%s" % (x["oaa"], x["naa"])
        ),
        "na_value": "NA",
        "hcdiff_derive": lambda x: (
            "NA" if x["type"] != "SNV" else "%s-%s" % (x["naa"], x["oaa"])
        ),
    },
}
